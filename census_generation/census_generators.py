...
class census_generator:
	def __init__(self, MCG_generators, arc_neighbours, automorph, MCG_must_contain, option, word_filter=basic_filter, manifold_filter=basic_filter):
		...
		self.table_generator = table_generator(MCG_generators, manifold_filter, option)
		self.option = option
	...
	def get_census_blocks(self, num_lines=None):
		...
		last_volume = 0
		num_census_blocks = 0
		main_block, previous_head, head, reskip_words = [], [], [], set()
		for line in open(self.option.good_file, 'r'):
			data = line.split('\t')
			word, volume = data[0], float(data[1])
			head.append(word)
			if abs(volume - last_volume) > self.option.VOLUME_ERROR:
				if len(main_block) + len(previous_head) > self.option.CHUNKSIZE_THIN:
					reskip_words = reskip_words.union(head)
					block = main_block + previous_head + head
					num_census_blocks += 1
					
					reskip_words = set(previous_head)
					main_block = []
				
				last_volume = volume
				main_block.extend(previous_head)
				previous_head = head
				head = []
		num_census_blocks += 1
		
		if self.option.SHOW_PROGRESS: print('%d blocks to check.' % num_census_blocks)
		
		last_volume = 0
		block_count = 0
		main_block, previous_head, head, reskip_words = [], [], [], set()
		for index, line in enumerate(open(self.option.good_file, 'r')):
			data = line.split('\t')
			word, volume = data[0], float(data[1])
			head.append(word)
			if abs(volume - last_volume) > self.option.VOLUME_ERROR:
				if len(main_block) + len(previous_head) > self.option.CHUNKSIZE_THIN:
					...
					block = main_block + previous_head + head
					block_count += 1
					yield (self, '%d - %d' % (block_count, num_census_blocks), block, reskip_words)
	...
	def build_census(self, depth, prebuilt=0, skip=0):
		...
		if prebuilt < 3:
			...
			clean_folder(self.option.problem_parts_dir)
		...
		if prebuilt < 4:
			...
			merge_sorted_files([self.option.good_parts + label for label in labels], self.option.good_file, key=lambda row: round(float(row[1]), self.option.VOLUME_ROUND))
			concatinate_files([self.option.problem_parts + label for label in labels], self.option.problem_file)
			concatinate_files([self.option.filtered_parts + label for label in labels], self.option.filtered_file)
		...
		if prebuilt < 6:
			...
			if self.option.SHOW_PROGRESS: print('Combining files.')
			labels = [I[1] for I in self.get_census_blocks(good_words)]
			concatinate_files([self.option.census_parts + label for label in labels], self.option.census_file)
			concatinate_files([self.option.not_problem_parts + label for label in labels], self.option.not_problem_file)
...
def determine_volumes_words_map(X):
	...
	table, problem_words, filtered_words = self.table_generator.convert_words_table(words)
	
	print_words_to_file(problem_words, self.option.problem_parts + label)
	print_words_to_file(filtered_words, self.option.filtered_parts + label)
	print_table_to_file(table, self.option.good_parts + label)
	
	return

def remove_duplicates_words_map(X):
	self, label, words, reskip_words = X
